...
import { ItFlowRibbonRenderer } from '../animations/itFlowRibbonRenderer.js';

export function ItFlowSection() {
  useEffect(() => {
    let rendererInstance = null;
    const sectionEl = document.querySelector('.it-flow-section');
    if (sectionEl) {
      try {
        rendererInstance = new ItFlowRibbonRenderer(sectionEl);
      } catch (err) {
        console.warn('ItFlowRibbonRenderer init error:', err);
      }
    }

    // 1. 3D Carved Groove & Rolling Sphere Scroll Handler
    const handlePathScroll = () => {
      ...
      if (rendererInstance) {
        rendererInstance.setScrollProgress(progress);
      }
    };
  ...
  return (
    <section className="it-flow-section">
      ...
      <div className="it-flow-cards">
        ...
        {/* Hidden SVG definition retained for path trajectory reference */}
        <svg className="it-network-paths w-embed" viewBox="0 0 740 2000" preserveAspectRatio="xMidYMid slice" aria-hidden="true" style={{ display: 'none' }}>
          <defs>
