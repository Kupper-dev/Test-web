...
export function ItFlowSection() {
  useEffect(() => {
    ...
    // 1. Multi-Layered SVG Energy Flow Scroll & Continuous Animation Handler
    const updateFlowAnimation = () => {
      ...
      const speedMult = 3200;
      const scrollOffset = -(speedMult * progress);
      const ambientPulse = -(elapsed * 220); // Continuous flow speed

      // A. Primary Core Energy Wave
      const coreLines = svgPaths.querySelectorAll("use.core-line");
      coreLines.forEach((u) => {
        u.style.strokeDashoffset = `${scrollOffset + ambientPulse}px`;
      });

      // B. Luminescent Ambient Glow Layer
      const glowLines = svgPaths.querySelectorAll("use.flow-glow-line");
      glowLines.forEach((u) => {
        u.style.strokeDashoffset = `${scrollOffset + ambientPulse * 0.9}px`;
      });

      // C. High-Speed Luminous Energy Packet Highlights
      const packetLines = svgPaths.querySelectorAll("use.flow-packet-line");
      packetLines.forEach((u) => {
        u.style.strokeDashoffset = `${scrollOffset * 1.5 + ambientPulse * 2.8}px`;
      });
    };
  ...
  return (
    <section className="it-flow-section">
      ...
      <div className="it-flow-cards">
        ...
        {/* Original SVG Signal paths with Multi-Layered Energy Flow */}
        <svg className="it-network-paths w-embed" viewBox="0 0 740 2000" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
          <defs>
            ...
            <linearGradient id="itSignalGradient" gradientUnits="objectBoundingBox" x1="0" y1="0" x2="0.3" y2="1">
          ...
          {/* 1. Outer track casing (Self-drawing background path - KEPT EXACTLY AS AUTHORIZED) */}
          ...
          {/* 2. Soft Luminescent Energy Aura Layer */}
          <use href="#itSignalPath1" className="flow-glow-line flow-glow-path-1" />
          <use href="#itSignalPath2" className="flow-glow-line flow-glow-path-2" />
          <use href="#itSignalPath3" className="flow-glow-line flow-glow-path-3" />

          {/* 3. Core Vibrant Energy Wave (Primary Flow Layer) */}
          ...
          {/* 4. High-Speed Luminous Energy Packet Highlights */}
          <use href="#itSignalPath1" className="flow-packet-line flow-packet-path-1" />
          <use href="#itSignalPath2" className="flow-packet-line flow-packet-path-2" />
          <use href="#itSignalPath3" className="flow-packet-line flow-packet-path-3" />
        </svg>
